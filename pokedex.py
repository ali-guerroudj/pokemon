...
import sys#importer le module sys, bibliotheque de jeu pour python
import json#importer le fichier json
import main#importer le fichier main pour permettre de naviguer d'une page a l'autre
...
def run(screen):
    pygame.init()#initialise tous les modules de pygames importés
    pygame.mixer.init()#initialise le module de mixage audio pour lecture des sons
#Pour gérer la taille de la fenetre
    SCREEN_WIDTH = 1100
    SCREEN_HEIGHT = 720
...
# Chargement de l'image pour le bouton gauche et ajustement de sa taille
...
# Chargement de l'image pour le bouton droit et ajustement de sa taille
...
# Chargement de l'image pour le bouton de retour et ajustement de sa taille
 ...
 # Création d'un rectangle pour détecter les clics sur le bouton de retour
    ...
    # Pour charger les images des Pokémon
    pokemon_images = []
    for pokemon in pokemon_data:
        image_path = f"ressource_graphique/pokemon/{pokemon['nom']}.png"
        pokemon_image = pygame.image.load(image_path)
        pokemon_image = pygame.transform.scale(pokemon_image, (100, 100))  # Pour ajuster la taille de l'image
    ...
    while run:
        ...
        if current_time - last_toggle_time > 300:  # Si plus de 300 millisecondes se sont écoulées depuis le dernier changement
            show_retour_button = not show_retour_button  # Pour Inverser l'état de l'affichage du bouton retour
            last_toggle_time = current_time  # Pour mettre à jour le temps du dernier changement
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if retour_button_rect.collidepoint(event.pos):
                    ...
                    logo_visible = False  # Pour cacher l'image gauche.png après un clic
                    logo_last_toggle_time = current_time  # Pour mettre à jour le temps du dernier changement de visibilité de l'image gauche.png
                elif pygame.Rect(second_image_position, second_image.get_size()).collidepoint(event.pos):
                    arrow_click_sound.play()
                    current_pokemon_index = (current_pokemon_index + 1) % len(pokemon_data)
                    second_image_visible = False  # Pour cacher l'image droite.png après un clic
                    second_image_last_toggle_time = current_time  # Pour mettre à jour le temps du dernier changement de visibilité de l'image droite.png
        ...
        # Pour afficher des attributs en rangées
        ...
        # Pour afficher l'image du Pokémon à gauche du Pokédex
        ...
        # Pour vérifier si l'image gauche.png doit être affichée ou non
        if not logo_visible:
        ...
        # Pour vérifier si l'image droite.png doit être affichée ou non
        ...
        # Pour permettre que le bouton retour quitte la page
